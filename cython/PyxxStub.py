...
class Generator:
  ...
  def bases (self, tp):
    ...
    return self.inherit_reduce (ancestors)
  ...
  def generate_types_pyxx (self, implementation_types):
    ...
    for tp in types:
      ...
      s += '  %-30s %-25s "dynamic_cast<%s*>" (void*) except NULL\n' % (u_typename + '*', tp.name + '_this1', c_typename)
    ...
    for tp in types:
      ...
      if tp.storage == Decls.RECORD:
        ...
        handle = 'deref (%s (self._this0))' % (tp.name + '_this1')
